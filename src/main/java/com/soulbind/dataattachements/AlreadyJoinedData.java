...
public record AlreadyJoinedData(Boolean value) {
    ...
    public static Codec<AlreadyJoinedData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Codec.BOOL.fieldOf("joined").forGetter(AlreadyJoinedData::value) // our object just has a list of floats
    ).apply(instance, AlreadyJoinedData::new)); // all the values we defined above are passed to the factory method we reference here
    ...
    public AlreadyJoinedData writeBoolean(Boolean value) {
        ...
        return new AlreadyJoinedData(value); // makes the list immutable to prevent accidental modification
    }
    ...
    public AlreadyJoinedData setNotJoined() {
        return new AlreadyJoinedData(false); // makes the list immutable to prevent accidental modification
    }

    public AlreadyJoinedData clear() { // clear method, just returns the default empty component
        return DEFAULT;
    }
}
